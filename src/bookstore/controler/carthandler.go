...
func AddBook2Cart(w http.ResponseWriter, r *http.Request) {
	...
	if flag {
		...
		if cart != nil {
			...
			// fmt.Println("根据图书的id获取的购物项是：", cartItem)
			if cartItem != nil {
				...
				for _, v := range cts {
					...
					// fmt.Println("cartItem.Book:", cartItem.Book)
					// 3.找到当前的购物项
			...
			// 不管之前是否有当前图书对应的购物项，都需要更新购物车中图书的总数量和总金额
			// 更新购物车信息到数据库中
			dao.UpdateCart(cart)
		} else {
			// 当前用户没有购物车，需要创建一个购物车并添加到数据库中
			// 1.创建购物车
			//    生成购物车id
