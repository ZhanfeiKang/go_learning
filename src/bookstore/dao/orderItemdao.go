...
func AddOrderItem(orderItem *model.OrderItem) error {
	...
	// 执行
	_, err := utils.Db.Exec(sqlStr, orderItem.Count, orderItem.Amount, orderItem.Title, orderItem.Amount, orderItem.Price, orderItem.ImgPath, orderItem.OrderId)
	if err != nil {
		return err
	}
	return nil
...
func GetOrderItemsByOrderID(orderID string) ([]*model.OrderItem, error) {
	...
	for rows.Next() {
		...
		rows.Scan(&orderItem.OrderItemID, &orderItem.Count, &orderItem.Amount,
			&orderItem.Title, &orderItem.Author, &orderItem.Price, &orderItem.ImgPath, &orderItem.OrderId)
		// 添加到切片中
